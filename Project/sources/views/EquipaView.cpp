...
EquipaInDTO EquipaView::getEquipa() {
    ...
    dto.pais = Utils::getPais("Pais");
    return dto;
}

int EquipaView::getId()       { return Utils::getNumber("ID da Equipa");  }
int EquipaView::getPilotoId() { return Utils::getNumber("ID do Piloto");  }
...
void EquipaView::printEquipa(const EquipaOutDTO& dto) {
    ...
    cout << "  Pilotos: ";
    if (dto.pilotoIds.empty()) cout << "(nenhum)";
    else for (int pid : dto.pilotoIds) cout << pid << " ";
    cout << "\n";
}

void EquipaView::printEquipas(list<EquipaOutDTO>& dtos) {
    if (dtos.empty()) { cout << "  Sem equipas registadas.\n"; return; }
    cout << "\n-- Lista de Equipas --\n";
    cout << string(70, '-') << "\n";
    for (auto& dto : dtos) printEquipa(dto);
    cout << string(70, '-') << "\n";
}